...
ZbarOrientationType = typing.Union[str, int, None]
...
# --- add this new dataclass (optional but clean) ---
@dataclass(frozen=True)
class QRZbarInfo:
    decoded: str | None
    zbar_orientation: ZbarOrientationType
...
class QReader:
    ...
    def detect_and_decode(
        self, image: np.ndarray, return_detections: bool = True, is_bgr: bool = False
    ) -> typing.Union[
        typing.Tuple[
            typing.Tuple[QRZbarInfo, ...],
            tuple[dict[str, np.ndarray | float | tuple[float | int, float | int]]],
        ],
        typing.Tuple[QRZbarInfo, ...],
    ]:
        """
        Like original detect_and_decode, but returns zbar orientation too.

        If return_detections is False:
            returns tuple[QRZbarInfo, ...]
        If return_detections is True:
            returns (tuple[QRZbarInfo, ...], detections)
        """
        detections = self.detect(image=image, is_bgr=is_bgr)

        infos: list[QRZbarInfo] = []
        for det in detections:
            decoded_str, z = self.decode_with_zbar(image=image, detection_result=det)
            zori = None if z is None else getattr(z, "orientation", None)
            infos.append(QRZbarInfo(decoded=decoded_str, zbar_orientation=zori))

        infos_t = tuple(infos)

        if return_detections:
            return infos_t, detections
        else:
            return infos_t
    ...
    def decode_with_zbar(
        self,
        image: np.ndarray,
        detection_result: dict,
    ):
        decoded = self._decode_qr_zbar(image=image, detection_result=detection_result)
        if len(decoded) == 0:
            return None, None  # (decoded_str, zbar_decoded)

        z = decoded[0].result
        decoded_str = z.data.decode("utf-8")
        for encoding in self.reencode_to:
            try:
                decoded_str = decoded_str.encode(encoding).decode("utf-8")
                break
